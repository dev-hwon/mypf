...
const RenderList = () => {
    ...
    if(loading) return <div>로딩중 ... <button onClick={fetchLists}>다시 불러오기</button></div>;
    if(error) return <div>에러가 발생했습니다. <button onClick={fetchLists}>다시 불러오기</button></div>;
            ...
            <ul>
                {newArr.map(list => (
                    <li key={list.id}>
                        ...
                        <div class="image1">{list.image1}</div>
                        <div class="image2">{list.image2}</div>
                        <div class="image3">{list.image3}</div>
